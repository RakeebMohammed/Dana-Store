...
totalAmount=0
  ...
  placeOrder: (details, products, total,userId) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      db.get().collection(collections.ORDER_COLLECTION).insertOne(orderObj).then(result=>{
 ...
 for(i=0;i<products.length;i++){
  db.get().collection(collections.PRODUCT_COLLECTION).updateMany({_id:products[i].item},{$inc:{stock:-products[i].quantity}})}
 if(details.payment==="Wallet"){
  let obj={Description:'Product purchase',
  Type:'Debit',
  Date: dated,
  Amount:total
 ...
 //})
  ...
  placeOrdersaved: (details, products, total,userId) => {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let status = details.payment === "COD" || 'Wallet' ? "Placed" : "Pending";
      let address= await db.get().collection(collections.USER_COLLECTION).aggregate([{$match:{_id:objectid(userId)}},{$project:{address:{$arrayElemAt:["$address",details.index]}}}]).toArray()
      console.log(address);
      let dated=moment().format('LL');
      orderObj = {
        UserId: objectid(userId),
...
resolve(result.insertedId);
  ...
  deleteOrder: (details,products) => {
    return new Promise((resolve, reject) => {
      ...
      console.log(details);
      let  history={Description:'Cancelling an order',
      Type:'Credit',
      ...
      db.get()
       ...
       {$set:{'PaymentStatus':'Cancelled'}}
        ...
        .then((result) => {
          console.log(result);
         for(i=0;i<products.length;i++){
          db.get().collection(collections.PRODUCT_COLLECTION).updateMany({_id:products[i].item},{$inc:{stock:products[i].quantity}}).then(res=>{
            console.log(res);
          })
         }
          if(details.method!='COD'){
          db.get().collection(collections.USER_COLLECTION).updateOne({_id: objectid(details.userId)},{$inc:{'Wallet':details.amount},$addToSet:{'WalletHistory':history}})
          }
        });
 ...
 console.log('pinnyum'+total)
  ...
  var expectedSignature = crypto.createHmac('sha256', 'Mn0WRR8jVHNsshE4GpBAxHrM')
  expectedSignature=expectedSignature          .update(details['order[id]'] + "|" +details['response[razorpay_payment_id]'] )
  ...
  changePaymentstatus:(details)=>{
    return new Promise((resolve, reject) => {
      db.get().collection(collections.ORDER_COLLECTION).updateOne({_id:objectid(details)},{$set:{
  ...
  deleteAddress:(details)=>{
    return new Promise((resolve, reject) => {
      db.get().collection(collections.USER_COLLECTION).updateOne({_id:objectid(details.userid)},{$pull:{address:{name:details.name
        ,email:details.email,phone_number:details.number,label:details.label,pincode:details.pin,city:details.city,state:details.state
      }}                                                                                        //   { $pull: { user_cart: { item: objectid(details.product) } } }                                    
      }).then(result=>{
        console.log(result);
      })
  ...
  // let user= await db.get().collection(collections.USER_COLLECTION).aggregate([{$match:{_id:objectid(code)}}]).toArray()
      let user= await db.get().collection(collections.USER_COLLECTION).findOne({$and:[{_id:objectid(code)},{'referralUsed':true}]})
  
     if(!user){

     let  transactionDetails={Description:'Reffering a friend',
     Type:'Credit',
     Date: moment().format('LL'),
     Amount:100

      }
      let  Details={Description:'Claiming a Refferal ',
     Type:'Credit',
     Date: moment().format('LL'),
     Amount:50

      }
      db.get().collection(collections.USER_COLLECTION).updateOne({_id:objectid(code)},{$set:{'referralUsed':true},$inc:{'Wallet':100}
      ,$push:{'WalletHistory':transactionDetails}}
      ).then(()=>{
        db.get( ).collection(collections.USER_COLLECTION).updateOne({_id:objectid(userId)}
        ,
     { $inc:{'Wallet':50},
     $push:{'WalletHistory':Details}} )
        console.log('sdhsh'); 
        resolve()
      }).catch(()=>{
        console.log('ibde');
...
reject()
  ...
  walletHistory:(userId)=>{
    return new Promise(async(resolve, reject) => {
      let history=await db.get().collection(collections.USER_COLLECTION).aggregate([{$match:{_id:objectid(userId)}},{$unwind:'$WalletHistory'},{$project:{'WalletHistory':1}}]).toArray()
...
resolve() }
  ...
  getWishlist:(userId=>{
    return new Promise(async(resolve, reject) => {
      let wish=await db.get().collection(collections.USER_COLLECTION).aggregate([{$match:{_id:objectid(userId)}},{$unwind:'$Wishlist'},
 ...
 //  console.log(wish);
  ...
  checkCoupon:((details,totalAmount)=>{
    return new Promise(async(resolve, reject) => {
      ...
      let couponExists=await db.get().collection(collections.COUPON_COLLECTION).findOne({name:details.coupon})
   if(couponExists)
   {
  ...
  removeWishlist:((productId,userId)=>{
      ...
      return new Promise(async(resolve, reject) => {
         ...
         db.get()
          ...
          let wish=await db.get().collection(collections.USER_COLLECTION).aggregate([{$match:{_id:objectid(userId)}},{$project:{Wishlist:1}}]).toArray() 
        console.log(wish);
    ...
    searchItem:(details)=>{
      ...
      let result=await  db.get().collection(collections.PRODUCT_COLLECTION).find({category:new RegExp(search , 'i')}).toArray()
        console.log(search);
        console.log(result);
        if(result.length!=0){
resolve(result)
 ...
 getProducts:(orderId)=>{
return new Promise(async(resolve, reject) => {
  let products=await db.get().collection(collections.ORDER_COLLECTION).aggregate([{$match:{_id:objectid(orderId)}},{$project:{'Products':1
  }}]).toArray()
 ...
 getStock:(productId=>{
  return new Promise(async(resolve, reject) => {
  let stock=await  db.get().collection(collections.PRODUCT_COLLECTION).aggregate([{$match:{_id:objectid(productId)}},{$project:{'stock':1}}]).toArray()
console.log(stock);
